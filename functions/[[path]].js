...
var authMiddleware = async (c, next) => {
  ...
  try {
    const payload = await verify2(token, c.env.JWT_SECRET || "dev-secret-change-me");
...
var AuthSchema = external_exports.object({ username: external_exports.string().min(1), password: external_exports.string().min(1) });
app.post("/api/auth/login", async (c) => {
  const body = await c.req.json();
  const parse2 = AuthSchema.safeParse(body);
  if (!parse2.success) return c.json({ error: "Invalid input" }, 400);
  const { username, password } = parse2.data;
...
var CreateLinkSchema = external_exports.object({
  slug: external_exports.string().min(1).max(64).regex(/^[a-z0-9-_]+$/),
  destinationUrl: external_exports.string().url().refine((u) => /^https?:\/\//i.test(u), "Only http(s) allowed"),
  isActive: external_exports.boolean().optional(),
  expiresAt: external_exports.string().datetime().optional(),
  clickLimit: external_exports.number().int().min(1).optional(),
...
app.post("/api/links", async (c) => {
  ...
  const body = await c.req.json();
  const parse2 = CreateLinkSchema.safeParse(body);
  if (!parse2.success) return c.json({ error: parse2.error.flatten() }, 400);
  const { slug, destinationUrl, isActive, expiresAt, clickLimit, tags } = parse2.data;
...
app.put("/api/links/:id", async (c) => {
  ...
  const data = await c.env.SHORT_URI_KV.get(`link:${id}`);
  if (!data) return c.json({ error: "not found" }, 404);
  const existingLink = JSON.parse(data);
  if (existingLink.ownerId !== "admin") {
    return c.json({ error: "forbidden" }, 403);
  }
  const updatedLink = { ...existingLink, ...parsed.data, updatedAt: (/* @__PURE__ */ new Date()).toISOString() };
...
app.get("/:slug", async (c) => {
  ...
  if (link.expiresAt && new Date(link.expiresAt) < /* @__PURE__ */ new Date()) return c.text("Expired", 410);
  try {
    ...
    const click = {
      ...
      referrer: ref,
      ipHash,
      country: c.req.header("cf-ipcountry") || void 0
    };
